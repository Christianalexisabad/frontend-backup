...
import "./Style.css";
import { EDIT_GROUP } from "../../../utility/Route";

const EditGroup = () => {

    const display = pathContains(EDIT_GROUP);
    const { id } = useParams();
    ...
    const fetchData = async () => {
        const response = await axios.get(getHost() + "/api/groups/get/"+ id + "/");
    ...
    const handleSubmit = (e) => { 
        ...
        if (isSuccess) {
            ...
            if (!name) {
                ...
                axios.patch(getHost() + "/api/groups/update/" + id + "/", {
                    name: name
                }).then(res => {  
