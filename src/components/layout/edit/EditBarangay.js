...
import "./Style.css";
import { EDIT_BARANGAY } from "../../../utility/Route";

const EditBarangay = () => {

    const display = pathContains(EDIT_BARANGAY);
    const { id } = useParams();
    ...
    const fetchData = async () => {
        const response = await axios.get(getHost() + "/api/barangays/get/"+ id + "/");
    ...
    const handleSubmit = (e) => { 
        ...
        if (isSuccess) {
            ...
            if (!name) {
                ...
                axios.patch(getHost() + "/api/barangays/update/" + id + "/", {
                    name: name
                }).then(res => {  
