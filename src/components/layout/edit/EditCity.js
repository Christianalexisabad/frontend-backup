...
import "./Style.css";
import { EDIT_CITY } from "../../../utility/Route";

const EditCity = () => {

    const display = pathContains(EDIT_CITY);
    const { id } = useParams();
    ...
    const fetchData = async () => {
        const response = await axios.get(getHost() + "/api/cities/get/"+ id + "/");
    ...
    const handleSubmit = (e) => { 
        ...
        if (isSuccess) {
            ...
            if (!name) {
                ...
                axios.patch(getHost() + "/api/cities/update/" + id + "/", {
                    name: name
                }).then(res => {  
