...
import { ADD_ROLE, EDIT_ROLE } from "../../../../../utility/Route";
import TableFooter from "./components/TableFooter";

export default function Role() {

    const endpoint = getHost() + "/api/roles/";
    const [data, setData] = useState([]);
    const [entry, setEntry] = useState(0);
    const [searchTerm, setSearchTerm] = useState("");

    const canEdit = hasPermission("can_edit_role");
    const canDelete = hasPermission("can_delete_role");
                        ...
                        <tr key={id}>
                            ...
                            <td>{title}</td>
                            {
                                ...
                                <td className="text-center">
                                    <EditButton permission="can_edit_role" to={EDIT_ROLE + id +"/"} />
                                    <DeleteButton permission="can_delete_role" from="roles" id={id}  name={title} />
                                </td>
                            }
                        </tr>
                    )
