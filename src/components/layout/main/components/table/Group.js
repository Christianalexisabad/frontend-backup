...
import { ADD_GROUP, EDIT_GROUP } from "../../../../../utility/Route";
import TableFooter from "./components/TableFooter";

export default function Group(props) {

    const endpoint = getHost() + "/api/groups/";
    const [data, setData] = useState([]);
    const [entry, setEntry] = useState(0);
    const [searchTerm, setSearchTerm] = useState("");

    const canEdit = hasPermission("can_edit_group");
    const canDelete = hasPermission("can_delete_group");
                        ...
                        <tr key={index}>
                            ...
                            <td>{name}</td>
                            {
                                ...
                                <td className="text-center">
                                    <EditButton permission="can_edit_group" to={EDIT_GROUP + id +"/"} />
                                    <DeleteButton permission="can_delete_group" from="groups" id={id}  name={name} />
                                </td>
                            }
                        </tr>
                    )
