...
export default function EmployeeLeaveBalance(props) {
    
    const display = true;
    const HOST = getHost();
    const endpoint = HOST + "/api/leave-balances/";
    ...
    const fetchLeaveTypes = async () => {
        const response = await axios.get(getHost() + "/api/leave-types/")
        const { data } = await response.data;
        setLeaveTypes(data); 
    }
    ...
    function renderTableHeader() {
        const header = [
            { id: "id", value: "no" },
            { id: "employee__employee_no", value: "employee no" },
            { id: "employee", value: "employee name" },
            { id: "leave_type", value: "leave type" },
            { id: "leave_type__duration", value: "applicable leave/duration(days)" },
            ...
            <tbody>
                {data.length > 0 && data.map((item, index) => {
                    return (
                        <tr key={index}>
                            <td>{index+=1}</td>
                            <td>{getData(item.employee, "employee_no", Employees)}</td>
                            <td>{getData(item.employee, "first_name", Employees) + " " + getData(item.employee, "sur_name", Employees)}</td>
                            <td>{getData(item.leave_type, "name", LeaveTypes)}</td>
                            <td>{getData(item.leave_type, "duration", LeaveTypes)}</td>
